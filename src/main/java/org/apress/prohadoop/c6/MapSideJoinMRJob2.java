...
public class MapSideJoinMRJob2 extends Configured implements Tool {
    ...
    public static class MapSideJoinMapper extends
        ...
        private void readAirports(Path p) throws Exception {
            
            List<String> lines = FileUtils.readLines(new File(p.toString()));
        ...
        private void readCarriers(Path p) throws Exception {
            List<String> lines = FileUtils.readLines(new File(p.toString()));
            for (String line : lines) {
        ...
        public void setup(Context context) {
            ...
            try {
                ...
                for (Path p : paths) {
                    if (p.toString().endsWith("airports.csv")) {
                        this.readAirports(p);
                    }
                    if (p.toString().endsWith("carriers.csv")) {
                        this.readCarriers(p);
                    }
                }
            } catch (Exception ex) {
    ...
    public int run(String[] allArgs) throws Exception {
        ...
        job.setJarByClass(MapSideJoinMRJob2.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        ToolRunner.run(new MapSideJoinMRJob2(), args);
    }
